...
def _gspread_client():
    ...
    info = {k: st.secrets["connections"]["gsheets"][k] for k in [
...
def _write_df(worksheet_name, df):
    gc  = _gspread_client()
...
def add_user(conn, username, pw_hash):
    ...
    _write_df("users", df); fetch_users.clear()
...
def send_friend_request(conn, from_user, to_user):
    ...
    _write_df("requests", df); fetch_requests.clear()
...
def send_message(conn, sender, receiver, content):
    ...
    _write_df("messages", df); fetch_messages.clear()
...
def update_online(conn, username):
    ...
    _write_df("online", df); fetch_online.clear()
